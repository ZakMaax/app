...
export default function ImageUploader({ maximumFiles, files = [], setFiles }: ImageUploaderProps) {
  ...
  // Remove file and revoke preview URL if it's a new file
  const removeFile = (index: number) => {
    const updatedFiles = [...files];
    const file = updatedFiles[index];
    // Only revoke if it's a new file (has a preview generated by URL.createObjectURL and size > 0)
    if (file.preview && file instanceof File && file.size > 0) {
  ...
  // Clean up preview URLs for new files on unmount
  useEffect(() => {
    return () => {
      files.forEach(file => {
        if (file.preview && file instanceof File && file.size > 0) {
  ...
  // Helper to get image src for both new and existing images
  const getImageSrc = (file: FilesWithPreview) =>
    file.preview ? file.preview : (file instanceof File && file.size > 0 ? URL.createObjectURL(file) : "");
